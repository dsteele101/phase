...
/// Printed Oracle text of Wand of Wonder (MTGJSON AtomicCards).
const WAND_OF_WONDER_ORACLE: &str = "{4}, {T}: Roll a d20. Each opponent exiles cards from the top of their library until they exile an instant or sorcery card, then shuffles the rest into their library. You may cast up to X instant and/or sorcery spells from among cards exiled this way without paying their mana costs.\n1—9 | X is one.\n10—19 | X is two.\n20 | X is three.";

/// CR 701.24a + CR 608.2c: In a 3-player game, Wand of Wonder's "Each opponent exiles cards...
/// then shuffles the rest into their library" runs per-opponent. P1 and P2 each exile until
/// an instant/sorcery and shuffle their respective libraries. The activator P0 does not shuffle.
#[test]
fn wand_of_wonder_shuffles_each_opponents_library() {
    const P2: PlayerId = PlayerId(2);
    let mut scenario = GameScenario::new_n_player(3, 42);
    scenario.at_phase(Phase::PreCombatMain);

    let wand = scenario
        .add_artifact_from_oracle(P0, "Wand of Wonder", WAND_OF_WONDER_ORACLE)
        .id();
    scenario.with_mana_pool(
        P0,
        vec![
    ...
    // Setup P1 library (top first: land, instant, deep)
    let _p1_deep = scenario.add_card_to_library_top(P1, "P1 Deep Card");
    let p1_hit = scenario
        .add_spell_to_library_top(P1, "P1 Instant", false)
        .as_instant()
        .id();
    let _p1_land = scenario.add_card_to_library_top(P1, "P1 Land");

    // Setup P2 library (top first: land, sorcery, deep)
    let _p2_deep = scenario.add_card_to_library_top(P2, "P2 Deep Card");
    let p2_hit = scenario
        .add_spell_to_library_top(P2, "P2 Sorcery", false)
        .as_sorcery()
        .id();
    let _p2_land = scenario.add_card_to_library_top(P2, "P2 Land");

    // P0 library card
    scenario.add_card_to_library_top(P0, "P0 Card");
    ...
    events.extend(
        runner
            ...
            .expect("activate Wand of Wonder")
            .events,
    );
    // Pass priority among all three players to resolve the activated ability on the stack
    ...
    assert_eq!(state.objects[&p1_hit].zone, Zone::Exile);
    assert_eq!(state.objects[&p2_hit].zone, Zone::Exile);
    assert_eq!(state.objects[&_p1_deep].zone, Zone::Library);
    assert_eq!(state.objects[&_p2_deep].zone, Zone::Library);
    ...
    assert_eq!(
        shuffles_for(P1),
        1,
        "opponent P1 must shuffle their library, got {events:?}"
    );
    assert_eq!(
        shuffles_for(P2),
        1,
        "opponent P2 must shuffle their library, got {events:?}"
    );
    assert_eq!(
        shuffles_for(P0),
        0,
        "activator P0 must not shuffle their library, got {events:?}"
    );
}

/// Printed Oracle text of Worldpurge (MTGJSON AtomicCards).
const WORLDPURGE_ORACLE: &str = "Return all permanents to their owners' hands. Each player chooses up to seven cards in their hand, then shuffles the rest into their library. Each player loses all unspent mana.";

/// CR 701.24a + CR 608.2c: In a 3-player game, Worldpurge's "Each player chooses...
/// then shuffles the rest into their library" runs per-player. Every player (P0, P1, P2)
/// shuffles their library.
#[test]
fn worldpurge_shuffles_each_players_library() {
    const P2: PlayerId = PlayerId(2);
    let mut scenario = GameScenario::new_n_player(3, 42);
    scenario.at_phase(Phase::PreCombatMain);

    let spell = scenario
        .add_spell_to_hand_from_oracle(P0, "Worldpurge", false, WORLDPURGE_ORACLE)
        .id();
    scenario.with_mana_pool(
        P0,
        vec![
    ...
    assert_eq!(
        shuffles_for(P0),
        1,
        "P0 must shuffle their library, got {events:?}"
    );
    assert_eq!(
        shuffles_for(P1),
        1,
        "P1 must shuffle their library, got {events:?}"
    );
    assert_eq!(
        shuffles_for(P2),
        1,
        "P2 must shuffle their library, got {events:?}"
    );
}
